...
int mdadm_write(uint32_t start_addr, uint32_t write_len, const uint8_t *write_buf) {
  ...
  int cloneaddr = start_addr; // clone a start_addr
  int looptime = 0; //track how many time loops
  ...
  while (needwrite >=1){ // if needread is now 0, means still have byte need to write, keep going. only stop if all bytes are done. 
   ...
   // printf("DiskID:%d.", CurrentDiskID);
    ...
    seekdisk = jbod (CurrentDiskID , 0, JBOD_SEEK_TO_DISK, 0);
    jbod_operation( seekdisk, NULL);
    seekblock = jbod(0,CurrentBlockID, JBOD_SEEK_TO_BLOCK, 0); // seek disk and block again to make sure
    jbod_operation(seekblock, NULL );
    ...
    block_spaceleft = 256 - (start_addr%256); // how many bytes left in this block.
    block_spaceused = (start_addr %256); // how many bytes have already used in this block.
    int blockcase = 2;  //second case, block left dont have enough space to put all write_len at once.
    int cloneaddr2 = start_addr; // use to debug
    ...
    jbod_operation(readblock, local);
    looptime =blockcase -2; // use to keep blockcase cause no print allowed in traces file.
    //printf("case:%d.", blockcase);

    
    seekdisk = jbod (CurrentDiskID , 0, JBOD_SEEK_TO_DISK, 0); // seek disk and block again, update the location for any loop.
    jbod_operation( seekdisk, NULL);
    //printf("DiskID:%d.", CurrentDiskID);
    seekblock = jbod(0,CurrentBlockID, JBOD_SEEK_TO_BLOCK, 0);
    jbod_operation(seekblock, NULL );

    cloneaddr = start_addr; 
    memcpy (local+block_spaceused, write_buf+writed, block_spaceleft);
    //memcpy (local, write_buf+block_spaceused, block_spaceleft);
    //start_addr +=needwrite;
    //printf("%d.", needwrite);
    start_addr = start_addr + block_spaceleft; // startaddr will update because new bytes are in.
    cloneaddr += block_spaceleft; 
    writed = writed + block_spaceleft; // writed will update as well cause some length is in.
    looptime++;  // loop time to keep track how many times loops.
    needwrite -=block_spaceleft; // write_len will - bytes already writed.
    cloneaddr2 += needwrite; // this was successfully use if write_buf+ this to local.
